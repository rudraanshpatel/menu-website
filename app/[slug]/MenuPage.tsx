      ...
      <div className="max-w-lg mx-auto">
        ...
        <div className="px-4 pb-2 flex gap-2">
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium border ${filterVeg === true
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium border ${filterVeg === false
            ...
            <div className="flex px-4 gap-1">
              {categories.map((cat) => (
                <button
                  ...
                  className={`px-4 py-2.5 text-sm font-medium whitespace-nowrap border-b-2 transition-colors ${activeCategory === cat.id
                    ? 'text-gray-900'
          ...
          {filteredItems.length === 0 ? (
            ...
            <div className="space-y-3">
              {filteredItems.map((item) => (
                <div
                  key={item.id}
                  className={`bg-white rounded-xl border border-gray-100 overflow-hidden ${!item.is_available ? 'opacity-50' : ''
                    }`}
                >
                  <div className="flex">
                    <div className="flex-1 p-4">
                      <div className="flex items-center gap-2 mb-1">
                        <span
                          className={`w-3 h-3 rounded-sm border-2 ${item.is_veg
                            ? 'border-green-600'
                            : 'border-red-500'
                            }`}
                        >
                          <span
                            className={`block w-1.5 h-1.5 rounded-full m-[1px] ${item.is_veg ? 'bg-green-600' : 'bg-red-500'
                              }`}
                          />
                        </span>
                        {!item.is_available && (
        ...
        {itemCount > 0 && (
          <div className="fixed bottom-0 left-0 right-0 z-30 p-3">
            <div
              className="max-w-lg mx-auto rounded-xl px-5 py-3 flex items-center justify-between text-white cursor-pointer"
              style={{ backgroundColor: accent }}
              onClick={() => window.location.href = `/${restaurant.slug}/checkout`}
            >
              <span className="text-sm font-bold">{itemCount} item{itemCount > 1 ? 's' : ''} · ₹{total}</span>
